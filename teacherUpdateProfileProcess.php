...
//********teacher update Profile process */
...
if (isset($_SESSION["tu"])) {
    ...
    if (isset($_FILES["image"])) {
        ...
        if (!in_array($file_ex, $allowed_image_extentions)) {
            ...
            $file_name = "resouses/user_profile_image/" . $_SESSION["tu"]["fname"] . "_" . uniqid() . $new_file_extention;
            move_uploaded_file($image["tmp_name"], $file_name);
            // echo("success");
            $image_rs = Database::search("SELECT * FROM `profile_image` WHERE 
            `user_id`='" . $_SESSION["tu"]["id"] . "'");
